...
export const VehicleTypes: string[] = [ 'BELREX', 'MSS', '5TON', 'OUV' ];
...
export class DatabaseService {
  ...
  public async init(email: string) {
    ...
    this.current.all_users = await this.list('user', ['company','==',this.current.user.company] );
  ...
  private bind(login:Login): void
  {
    ...
    query = login.user.is_admin ? query : // Admin gets everything
      login.user.is_commander ? 
      query.where("commander", "==", login.user.email) : 
      query.where("driver", "==", login.user.email);
...
/*
export interface Vehicle  // TODO: This is no longer used...
{
  created: string;
  id: string;  // vehicle number, identifier
  type: string; // valid types: 'BELREX', 'MSS', '5TON', 'OUV'
}
*/
